...
class RiskTests(unittest.TestCase):
    ...
    def test_accounts_must_cover_selected_exchanges(self) -> None:
        cfg = load_config("config.example.toml")
        accounts = [
            AccountSnapshot.from_api({"exchange": "aster", "availableBalance": "100", "type": "perps", "token": "USDT"}),
            AccountSnapshot.from_api({"exchange": "hyperliquid", "availableBalance": "100", "type": "perps", "token": "USDC"}),
        ]
    ...
    def test_non_hyperliquid_spot_balance_does_not_count_for_perps(self) -> None:
        cfg = load_config("config.example.toml")
        accounts = [
            AccountSnapshot.from_api({"exchange": "aster", "availableBalance": "100", "type": "spot", "token": "USDC"}),
            AccountSnapshot.from_api({"exchange": "lighter", "availableBalance": "100", "type": "perps", "token": "USDC"}),
        ]
    ...
    def test_hyperliquid_unified_spot_usdc_counts_for_perps(self) -> None:
        cfg = load_config("config.example.toml")
        accounts = [
            AccountSnapshot.from_api({"exchange": "hyperliquid", "availableBalance": "100", "type": "spot", "token": "USDC"}),
            AccountSnapshot.from_api({"exchange": "lighter", "availableBalance": "100", "type": "perps", "token": "USDC"}),
        ]
    ...
    def test_quote_size_uses_leverage_adjusted_margin_requirement(self) -> None:
        cfg = load_config("config.example.toml")
        risk = RiskEngine(cfg)
        self.assertEqual(risk.required_margin_per_exchange(), Decimal("82.50"))

        accounts = [
            AccountSnapshot.from_api({"exchange": "hyperliquid", "availableBalance": "82", "type": "spot", "token": "USDC"}),
            AccountSnapshot.from_api({"exchange": "lighter", "availableBalance": "100", "type": "perps", "token": "USDC"}),
        ]
        with self.assertRaises(RiskReject):
